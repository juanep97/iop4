...
def main():
    ...
    if not args.list_only:
        if len(epochnames_to_process) > 0:
            ...
            epochnames_to_process, _ = sorted(zip(*[(epochname, Epoch.epochname_to_tel_night(epochname)[1].strftime("%Y-%m-%d")) for epochname in epochnames_to_process]), reverse=True)
